...
- **v2.6.0（2026年8月14日）**：
  - 将彩色日志系统与 Uvicorn 同步。 Live Timer 集成测量界面上每个 RAG 阶段的执行时间，并显示每个答案的性能元数据信息（延迟、令牌使用情况）。 
  - 深度优化后端云性能和可靠性。 
  - 优化 `/status` 端点，直接运行记录计数（`len(vector_db)`），而不是下载整个块，将速度从秒提升到小于 5ms，彻底解决 UI 闪烁和连接丢失的现象。 
  - 在启动时设置自我修复摄取：如果 Qdrant Cloud 上的集合丢失数据，则自动检测并干净地重新填充。 
  - 通过将网络超时增加到 120 秒并自动调整批量大小，修复将数据上传到 Qdrant Cloud 时的“读取操作超时”错误。 
  - 将Gemini的所有内容生成和Reranking功能转换为与最新Google GenAI SDK的自动函数调用（AFC）机制兼容的Chat对象，完全消除日志中的警告。 
  - UI 改进：加载页面时添加黄色“正在连接...”中间状态，在“正在回答...”问题生成状态标签旁边集成视觉响应时间显示。

- **v2.6.1（2026年8月15日）**：清理源代码，删除未使用的组件